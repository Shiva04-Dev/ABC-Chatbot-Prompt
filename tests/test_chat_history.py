...
def test_second_turn_includes_prior_exchange_as_history(client, recording_client):
    client.post("/chat", json={"session_id": sid("s1"), "message": "Hello"})
    client.post("/chat", json={"session_id": sid("s1"), "message": "Tell me more"})
...
def test_history_persisted_in_session_store_after_reply(client, sessions):
    client.post("/chat", json={"session_id": sid("s1"), "message": "Hello"})
    assert sessions.get_history(sid("s1")) == [
...
def test_history_stays_capped_across_many_turns(client, sessions):
    for i in range(10):
        client.post("/chat", json={"session_id": sid("s1"), "message": f"message {i}"})

    assert len(sessions.get_history(sid("s1"))) == MAX_HISTORY_MESSAGES


def test_history_is_not_shared_across_sessions(client, sessions):
    client.post("/chat", json={"session_id": sid("a"), "message": "Hello from a"})
    client.post("/chat", json={"session_id": sid("b"), "message": "Hello from b"})

    assert sessions.get_history(sid("a")) == [
        {"role": "user", "content": "Hello from a"},
        {"role": "assistant", "content": "reply 1"},
    ]
    assert sessions.get_history(sid("b")) == [
